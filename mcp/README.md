...
* its own **`agent-mcp` dashboard**, because the rich **Agent Sessions** view
  (`agent-decisions`) is generated by the daemon that an MCP-only user does not have

### One dashboard when the daemon is present

If the PowerShell daemon is installed it owns the **Agent Sessions** dashboard
(`agent-decisions`) and renders every
...
`agent-mcp` itself, so an MCP-only install still works unchanged. Cards are merged
