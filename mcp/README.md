...
* its own **`copilot-mcp` dashboard**, because the rich **Agent Sessions** view
  (`copilot-decisions`) is generated by the daemon that an MCP-only user does not have

### One dashboard when the daemon is present

If the PowerShell daemon is installed it owns the **Agent Sessions** dashboard
(`copilot-decisions`) and renders every
