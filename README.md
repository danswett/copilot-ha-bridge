...
disappear rather than lingering unavailable beside their replacements, and the
**Detailed activity** toggle is migrated with its on/off value carried across — a
delete-and-recreate would silently reset it to off.
...
process vanished. With **Detailed activity** on and `model_reasoning_effort` set, the
